    ...
    <section className="flex flex-1 items-start justify-center px-8 pt-15 pb-5 sm:px-8 md:px-10 lg:px-20 lg:py-20">
      <div className="w-full max-w-7xl space-y-6 md:space-y-8">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-sm md:text-md leading-5 text-muted-foreground">
            Start by selecting the courses you want to enroll in
          </p>
        </div>
        {selectedCourses.length > 0 && (
          <div className="rounded-sm md:rounded-lg border border-border bg-card p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <p className="text-sm md:text-md md:mt-1 text-muted-foreground">
              ...
              {selectedCourses.map((course) => {
                return (
                  <button
                    ...
                    className="cursor-pointer flex items-center gap-2 rounded-sm md:rounded-md border border-border bg-muted/40 px-4 py-2 text-xs md:text-sm transition-all hover:bg-muted"
                  >
        ...
        <div className="relative">
          <Search className="absolute left-4 top-1/2 h-3.5 w-3.5 md:h-4 md:w-4 -translate-y-1/2 text-muted-foreground" />
          <input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search courses or course codes..."
            className="w-full rounded-md border border-input bg-card py-3 pl-11 pr-5 text-[13px] md:text-sm outline-none transition-all placeholder:text-muted-foreground focus:ring-1 focus:ring-ring focus:ring-offset-1 focus:ring-offset-background"
          />
        </div>
